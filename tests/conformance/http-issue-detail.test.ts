...
describe("conformance: issue detail payload", () => {
  ...
  it("returns recent_events in issue detail response", async () => {
    const recentEvents = [
    ...
    expect(payload.running).toBeDefined();
    // @ts-ignore
    expect(payload.writeback_status).toBeDefined();
    // @ts-ignore
    expect(payload.verifications).toBeDefined();
  });
